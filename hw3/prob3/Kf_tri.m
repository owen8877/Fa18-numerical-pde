...
function Au = Afunc(u, const, beta, h, N)
    ...
    Au(1, N+1) = (u(1, N+1) * ns + (u(1, N) + u(2, N+1)) * na) * (h * beta) + (u(1, N) + u(2, N+1)) * me/2 + u(1, N+1) * ms/3;
    Au(N+1, N+1) = (u(N+1, N+1) * ns + (u(N+1, N) + u(N, N+1)) * na) * (h * beta) + (u(N+1, N) + u(N, N+1)) * me/2 + u(N+1, N+1) * ms/3;
...
function fv = ffunc(f, u0, g_x0, g_1y, g_x1, const, h, N)
    ...
    fv(1, 2) = fv(1, 2) - u0(1)*me;
    fv(N+1, 2) = fv(N+1, 2) - u0(N+1)*me;
    for y = 1:N-1
        fv(y+1, N+1) = ( ...
            g_1y(y+1) * ns ...
          + (g_1y(y) + g_1y(y+2)) * na ...
    ...
    fv(1, N+1) = ((g_x0(N+1)+g_1y(1)) * ns/2 + (g_x0(N) + g_1y(2)) * na) * h + (f(1, N) + f(2, N+1)) * le/2 * h^2;
